...
class SettingsWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.nickname_inp.setStyleSheet("QLineEdit { background-color: #222; border: 1px solid #444; border-radius: 4px; padding: 5px; color: #fff; }")
        ...
        timer_tab_layout.addWidget(QLabel("貼心提醒：雙擊 F1~F9 可快速切換配置組", styleSheet="color: #888; font-size: 11px; margin-bottom: 5px;"))
        ...
        ship_btn = QPushButton("🚢 開始下班船班倒數"); ship_btn.setStyleSheet(btn_common_style); ship_btn.clicked.connect(self.start_ship_timer); ship_layout.addWidget(ship_btn)
        
        elevator_row = QHBoxLayout()
        ed_btn = QPushButton("🔻 赫爾奧斯塔 (下樓)"); ed_btn.setStyleSheet(btn_common_style); ed_btn.clicked.connect(lambda: self.start_elevator_timer("down"))
        eu_btn = QPushButton("🔺 赫爾奧斯塔 (上樓)"); eu_btn.setStyleSheet(btn_common_style); eu_btn.clicked.connect(lambda: self.start_elevator_timer("up"))
        elevator_row.addWidget(ed_btn); elevator_row.addWidget(eu_btn); ship_layout.addLayout(elevator_row)
        ship_group.setLayout(ship_layout); timer_tab_layout.addWidget(ship_group)
        ...
        reset_exp_btn = QPushButton("🔄 重新開始紀錄 (歸零重算)"); reset_exp_btn.setStyleSheet(btn_common_style); reset_exp_btn.clicked.connect(self.on_reset_exp_clicked); exp_tab_layout.addWidget(reset_exp_btn)
        
        opacity_info = QLabel("✨ 面板背景透明度"); opacity_info.setStyleSheet("color: #aaa; font-size: 11px; margin-top: 15px;"); exp_tab_layout.addWidget(opacity_info)
        opacity_row = QHBoxLayout()
        self.opacity_val_lbl = QLabel(f"{int(config.get('opacity', 0.5)*100)}%"); self.opacity_val_lbl.setStyleSheet("color: #ffd700; font-weight: bold;")
        self.opacity_slider = QSlider(Qt.Orientation.Horizontal); self.opacity_slider.setRange(10, 100); self.opacity_slider.setValue(int(config.get("opacity", 0.5)*100)); self.opacity_slider.valueChanged.connect(self.on_opacity_changed)
        opacity_row.addWidget(self.opacity_slider); opacity_row.addWidget(self.opacity_val_lbl); exp_tab_layout.addLayout(opacity_row)
        ...
        move_rjpq_btn = QPushButton("🔱 調整羅茱面板位置"); move_rjpq_btn.setStyleSheet(btn_common_style); move_rjpq_btn.clicked.connect(self.toggle_rjpq_handle)
        ...
        hk_grid = QGridLayout()
        hk_labels = {
            "exp_toggle": "📊 顯示/隱藏經驗面板", "exp_pause": "⏸ 暫停/恢復紀錄 (F11)", 
            "reset": "🧹 重置清空所有計時器 (F9)", "exp_report": "📸 產出經驗成果圖 (F12)",
            "rjpq_1": "🎮 羅茱 - 標記位置 1", "rjpq_2": "🎮 羅茱 - 標記位置 2",
            "rjpq_3": "🎮 羅茱 - 標記位置 3", "rjpq_4": "🎮 羅茱 - 標記位置 4",
        ...
        for idx, (hk_id, txt) in enumerate(hk_labels.items()):
            ...
            btn = QPushButton("無" if raw_val == "NONE" else raw_val); btn.setFixedWidth(100); btn.setStyleSheet(btn_common_style)
        ...
        exit_btn = QPushButton("🛑 關閉整個輔助"); exit_btn.clicked.connect(QApplication.instance().quit); self.layout.addWidget(exit_btn)
    ...
    def keyPressEvent(self, event):
        key_code = event.key(); is_escape = (key_code == Qt.Key.Key_Escape)
        if self.recording_global_key:
            key_name = "none" if is_escape else self.qt_key_to_name(event)
            if key_name:
                config = ConfigManager.load_config(); config["hotkeys"][self.recording_global_key] = key_name; ConfigManager.save_config(config)
                self.global_hk_buttons[self.recording_global_key].setText("無" if key_name == "none" else key_name.upper())
                self.global_hk_buttons[self.recording_global_key].setStyleSheet("")
                self.recording_global_key = None
            return

        if self.is_recording:
            if is_escape: self.toggle_recording(); return
            key_name = self.qt_key_to_name(event)
            if key_name:
                p_key = self.profile_box.currentData() or "F1"; config = ConfigManager.load_config(); triggers = self.capture_ui_data()
                config["profiles"][p_key]["triggers"] = triggers
                if key_name not in triggers:
                    config["profiles"][p_key]["triggers"][key_name] = {"seconds": 300, "icon": ""}; ConfigManager.save_config(config)
                self.refresh_items(); self.toggle_recording()

    def capture_ui_data(self):
        data = {}
        for k, ui in self.trigger_data.items():
            try: data[k] = {"seconds": int(ui["inp"].text()), "icon": ui["icon"], "sound": ui["cb_sound"].isChecked()}
            except: data[k] = {"seconds": 300, "icon": ui["icon"], "sound": True}
        return data

    def update_profile_dropdown(self):
        self.profile_box.blockSignals(True); self.profile_box.clear()
        config = ConfigManager.load_config(); active = config.get("active_profile", "F1")
        for i in range(1, 10):
            key = f"F{i}"; name = config["profiles"][key].get("name", f"Profile {i}")
            self.profile_box.addItem(f"{key}: {name}", key)
        for i in range(self.profile_box.count()):
            if self.profile_box.itemData(i) == active:
                self.profile_box.setCurrentIndex(i); self.nickname_inp.setText(config["profiles"][active].get("name", "")); break
        self.profile_box.blockSignals(False); self.profile_box.currentIndexChanged.connect(self.switch_profile_ui)

    def on_nickname_changed(self, text):
        key = self.profile_box.currentData()
        if not key: return
        config = ConfigManager.load_config(); config["profiles"][key]["name"] = text; ConfigManager.save_config(config)
        self.profile_box.blockSignals(True); self.profile_box.setItemText(self.profile_box.currentIndex(), f"{key}: {text}"); self.profile_box.blockSignals(False)
    ...
    def qt_key_to_name(self, event):
        ...
        try: return chr(code).lower() if 32 <= code <= 126 else None
        except: return None

    def toggle_recording(self):
        self.is_recording = not self.is_recording
        if self.is_recording:
             self.record_btn.setText("🔴 請按鍵盤錄製中..."); self.record_btn.setStyleSheet("background-color: #c62828; color: white;")
        else: self.record_btn.setText("➕ 新增按鍵 (點我後按鍵盤)"); self.record_btn.setStyleSheet("")
    ...
    def toggle_timer_handle(self):
        ...
        if self.handle.isVisible(): self.handle.hide()
        else:
            if self.overlay:
                geo = self.overlay.geometry(); cx = geo.x() + geo.width() // 2 + self.overlay.x_offset; cy = geo.y() + geo.height() // 2 + self.overlay.y_offset
                self.handle.move(cx - 30, cy - 30); self.overlay.show_preview = True; self.overlay.update()
            self.handle.show(); self.handle.raise_()

    def toggle_exp_handle(self):
        if not self.exp_handle:
            c = ConfigManager.load_config(); ox, oy = c.get("exp_offset", [0, 0]); self.exp_handle = PositionHandle()
            if self.overlay: self.exp_handle.move(self.overlay.rect().center().x() + ox - 30, self.overlay.rect().center().y() + oy - 150); self.exp_handle.position_changed.connect(self.overlay.update_exp_offset)
            self.exp_handle.show()
        else: self.exp_handle.close(); self.exp_handle = None; self.save_and_close()

    def toggle_rjpq_handle(self):
        if not self.rjpq_handle:
            c = ConfigManager.load_config(); ox, oy = c.get("rjpq_offset", [-200, 0]); self.rjpq_handle = PositionHandle()
            center = self.overlay.mapToGlobal(self.overlay.rect().center())
            self.rjpq_handle.move(center.x() + ox - 30, center.y() + oy - 30); self.rjpq_handle.position_changed.connect(self.overlay.update_rjpq_offset); self.rjpq_handle.show()
            if self.overlay: self.overlay.show_rjpq_panel = True; self.overlay.update()
        else: self.rjpq_handle.close(); self.rjpq_handle = None; self.save_and_close()

    def start_ship_timer(self):
        now = datetime.datetime.now(); rem_min = 10 - (now.minute % 10); total_sec = (rem_min * 60) - now.second
        if total_sec <= 0: total_sec = 600
        if self.overlay: self.overlay.timer_request.emit("Ship", total_sec, "buff_pngs/Others/ship_icon.png", True)

    def start_elevator_timer(self, dir):
        now = datetime.datetime.now()
        if dir == "down": total_sec = ((4 - (now.minute % 4)) * 60) - now.second
    ...
    def refresh_items(self):
        while self.scroll_layout.count():
            ...
            if w: w.deleteLater()
        config = ConfigManager.load_config(); p_key = self.profile_box.currentData() or "F1"; p_data = config["profiles"].get(p_key, {"triggers": {}})
        self.trigger_data = {}
        for key, data in p_data["triggers"].items():
            if isinstance(data, int): data = {"seconds": data, "icon": ""}
            row = QFrame(); row.setFixedHeight(50); row.setStyleSheet("background-color: #1e1e1e; border-radius: 4px;"); layout = QHBoxLayout(row)
            lbl = QLabel(key.upper()); lbl.setStyleSheet("color: #ffd700;"); lbl.setFixedWidth(60); layout.addWidget(lbl)
            btn = QPushButton(); btn.setFixedSize(32, 32); self.update_icon_button(btn, data.get("icon", "")); btn.clicked.connect(lambda checked, k=key, b=btn: self.pick_icon(k, b)); layout.addWidget(btn)
            inp = QLineEdit(str(data.get("seconds", 300))); inp.setFixedWidth(35); layout.addWidget(inp); layout.addWidget(QLabel("秒"))
            cb = QCheckBox("20s音效"); cb.setChecked(data.get("sound", True)); layout.addWidget(cb); layout.addStretch()
            del_btn = QPushButton("🗑️"); del_btn.setFixedWidth(30); del_btn.clicked.connect(lambda checked, k=key: self.delete_key(k)); layout.addWidget(del_btn)
            self.trigger_data[key] = {"inp": inp, "icon": data.get("icon", ""), "cb_sound": cb}; self.scroll_layout.addWidget(row)
        self.scroll_layout.addStretch()

    def delete_key(self, key):
        config = ConfigManager.load_config(); triggers = self.capture_ui_data()
        if key in triggers:
            del triggers[key]; p_key = self.profile_box.currentData() or "F1"; config["profiles"][p_key]["triggers"] = triggers; ConfigManager.save_config(config); self.refresh_items()

    def update_icon_button(self, btn, path):
        real = resource_path(path) if path and not os.path.isabs(path) else path
        if real and os.path.exists(real):
            btn.setIcon(QIcon(QPixmap(real).scaled(24, 24, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))); btn.setIconSize(QSize(24, 24)); btn.setText("")
        else: btn.setIcon(QIcon()); btn.setText("🖼️")

    def pick_icon(self, key, btn):
        dlg = IconSelectorDialog(self)
        if dlg.exec(): path = dlg.selected_icon; self.trigger_data[key]["icon"] = path; self.update_icon_button(btn, path)
    ...
    def save_and_close(self):
        ...
        config["profiles"][p_key]["triggers"] = triggers; config["profiles"][p_key]["name"] = self.nickname_inp.text()
        if self.overlay:
            config["offset"] = [self.overlay.x_offset, self.overlay.y_offset]; config["exp_offset"] = [self.overlay.exp_x_offset, self.overlay.exp_y_offset]
            config["show_exp"] = self.overlay.show_exp_panel; config["show_money_log"] = self.overlay.show_money_log
            config["show_debug"] = self.overlay.show_debug; config["opacity"] = self.overlay.base_opacity; config["rjpq_offset"] = [self.overlay.rjpq_x_offset, self.overlay.rjpq_y_offset]
        ConfigManager.save_config(config); self.config_updated.emit(); self.hide()
        if self.handle: self.handle.hide()
        if self.exp_handle: self.exp_handle.hide()
        if self.rjpq_handle: self.rjpq_handle.hide()
