...
class SettingsWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.money_active_cb = QCheckBox("開啟楓幣記錄（需打開道具欄）")
        ...
        self.opacity_val_lbl = QLabel(f"{int(config.get('opacity', 0.75) * 100)}%")
        self.opacity_val_lbl.setStyleSheet("color: #ffd700; font-weight: bold;")
        self.opacity_slider = QSlider(Qt.Orientation.Horizontal)
        self.opacity_slider.setRange(10, 100)
        self.opacity_slider.setValue(int(config.get("opacity", 0.75) * 100))
