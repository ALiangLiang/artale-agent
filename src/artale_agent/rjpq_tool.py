...
try:
    ...
    temp_logger.info("[RJPQ] Qt6 SSL Support: %s", ssl_ok)
    if ssl_ok:
        temp_logger.info("[RJPQ] SSL Library Build: %s", QSslSocket.sslLibraryBuildVersionString())
        temp_logger.info("[RJPQ] SSL Library Runtime: %s", QSslSocket.sslLibraryVersionString())
except Exception:
    pass
...
class RJPQSyncClient(QObject):
    ...
    def connect_to_room(self, code, pwd):
        try:
            ...
            logger.error("[RJPQ] Connection failure: %s", e)
    ...
    def on_error(self, error):
        ...
        logger.error("[RJPQ] WebSocket Internal Error: %s (Code: %s)", err_str, error)
...
class RJPQTabContent(QWidget):
    ...
    def init_ui(self):
        ...
        self.code_inp.setStyleSheet(
            "background: #222; color: #ffd700; border: 1px solid #444; border-radius: 4px; padding: 4px;"
        )
        ...
        self.create_btn.setStyleSheet(
            "QPushButton { background: #333; color: #88ccff; font-weight: bold; border-radius: 4px; height: 26px; }"
        )
        ...
        ctrl_vbox = QVBoxLayout(self.overlay_ctrl)
        ctrl_vbox.setContentsMargins(10, 0, 10, 5)
        ...
        for i in range(4):
            ...
            btn.setStyleSheet(
                f"QPushButton {{ background: #222; color: {char_colors[i]}; border: 1px solid #444; border-radius: 4px; font-family: {platform_font_family()}; }} "
        ...
        dashboard_title.setStyleSheet(
            "color: #888; font-size: 10px; font-weight: bold; margin-bottom: 5px;"
        )
        ...
        for row in range(10):
            ...
            row_label.setStyleSheet(
                f"color: #444; font-weight: bold; font-family: {platform_font_family()};"
            )
    ...
    def update_status(self, connected):
        try:
            ...
            if connected:
                ...
                self.conn_btn.setStyleSheet(
                    "QPushButton { background: #c62828; color: #fff; font-weight: bold; border-radius: 4px; height: 26px; }"
                )
            ...
            logger.error("[RJPQ] update_status error: %s", e)
    ...
    def hide_ui_on_disconnect(self):
        ...
        if not self.client.is_connected:
            ...
            self.char_widget.setVisible(False)
            self.grid_widget.setVisible(False)
            for btn in self.char_btns:
                btn.setChecked(False)
            
            # 清空數據與網格
            self.current_data = [4] * 40
            self.update_grid([4] * 40)
            
            # 隱藏遊戲內面板
            self.client.overlay_toggle_request.emit(False)
    ...
    def platform_clicked(self, index):
        ...
        if current_val == self.selected_color:
            self.client.send_action(
                {"type": "unmark", "index": index, "color": self.selected_color}
            )
        elif current_val == 4:
            self.client.send_action(
    ...
    def on_overlay_cb_toggled(self, checked):
        if not checked:
            self.client.overlay_toggle_request.emit(False)
            return
    ...
    def update_grid(self, data):
        try:
            if not data or len(data) < 40:
                logger.warning("[RJPQ] Malformed grid data: %s", data)
                return
            ...
            logger.error("[RJPQ] update_grid error: %s", e)
